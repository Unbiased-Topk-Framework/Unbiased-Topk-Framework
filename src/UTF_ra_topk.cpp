...
void test_topk(int bucket_length, int topk, int rep_time, int tot_flow, int heavy_bucnum, int light_bucnum){	
	...
	double avg_AAE_unbiased = 0, avg_ARE_under = 0, avg_ARE_unbiased = 0, avg_ARE_over = 0, avg_f1 = 0, avg_under = 0, avg_over = 0, avg_equal = 0, avg_deviate = 0, avg_badcase = 0, avg_throughput = 0, avg_precision = 0;

	for (int t = 0; t < rep_time; t++){
		printf("%d ", t);

		s = new Sketch_ra(1, 1. / 1.08, 1, heavy_bucnum, bucket_length, light_bucnum);
		...
		for (i = 0; i < topk; i++){
			...
			if (tmp_hit){
				...
				#ifdef DEBUG_MODE
					fprintf(fout, "%d %d ", flow[i].cnt, res);
				#endif
			}
...
int main(int argc, const char* argv[]){
	...
	printf("UTF + Random Admission:\n");
	fprintf(fout, "UTF + Random Admission:\n");
