...
Finding top-$𝐾$ frequent items is a fundamental problem in data stream processing, and the unbiasedness of estimated frequency is an important property in this problem. However, to the best of our knowledge, no existing algorithm achieves unbiasedness o f reported top-k items, which is the property we care in practice. In this paper, we propose the Unbiased Top-$K$ Framework (UTF for short). Our key idea is to decouple the processes of finding top-$K$ items and estimating item frequencies, so as to make them uncorrelated, achieving unbiasedness for reported top-$K$ items. Then we propose an optimization technique, namely error freezing,to raise the accuracy of UTF. Our experimental results show that,compared with the state-of-the-art, UTF decreases its error (ARE)by up to $1.7 \times 10^4$ times ($757$ times in average) in finding top-$K$ frequent items. 
...
We also provide some of our datasets for our experiments. We made them publicly available at Google drive. See `Usage` section for details.
...
We conduct experiments on four datasets: CAIDA, Webpage, Network, and Synthetic datasets. CAIDA, Webpage and Network datasets are open-sourced real-world datasets, while Synthetic datasets are generated by Zip-f distributions.

We upload CAIDA datasets used for our experiment at Google drive: `https://`. You can also obtain Webpage and Network datasets in their respective websites.
...
After that, you should modify `src/run.sh`, and then run `src/run.sh`. All the programs are expected to run automatically and produce the results in a file specified in `src/test_cases.txt`.
